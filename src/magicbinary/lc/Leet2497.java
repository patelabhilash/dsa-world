...
public class Leet2497 {
    
    public int maxStarSum(int[] vals, int[][] edges, int k) {
        PriorityQueue<Integer>[] adjlist = IntStream.range(0, vals.length).boxed().map(x -> new PriorityQueue<Integer>(Comparator.reverseOrder())).toArray(PriorityQueue[]::new);
        for(int[] e : edges){
            if(vals[e[1]] > 0 && (adjlist[e[0]].size()<=k || (adjlist[e[0]].size()>k && adjlist[e[0]].peek()<=vals[e[1]]))) adjlist[e[0]].add(vals[e[1]]);
            if(vals[e[0]] > 0 && (adjlist[e[1]].size()<=k || (adjlist[e[0]].size()>k && adjlist[e[1]].peek()<=vals[e[0]]))) adjlist[e[1]].add(vals[e[0]]);
        }
        for (int index = 0; index < adjlist.length; index++) {
            System.out.print(index + "\t" + vals[index]+ "\t");
            adjlist[index].forEach(x -> System.out.print(x+ "\t"));
            System.out.println();
        }
        ...
        for(int i=0;i<vals.length; i++){
            tempMax = vals[i];
            for(int j=0;j<k && (adjlist[i].size()>0);j++){
                tempMax+= adjlist[i].poll(); 
            }
            if(tempMax > max) {System.out.println(i); max = tempMax;};    
        }
        return max;
    }
   ...
   public int maxStarSum2(int[] vals, int[][] edges, int k) {
        PriorityQueue<Integer>[] adjlist = IntStream.range(0, vals.length).boxed().map(x -> new PriorityQueue<Integer>(Comparator.reverseOrder())).toArray(PriorityQueue[]::new);
        for(int[] e : edges){
            if(vals[e[1]] > 0 ) adjlist[e[0]].add(vals[e[1]]);
            if(vals[e[0]] > 0  ) adjlist[e[1]].add(vals[e[0]]);
        }
